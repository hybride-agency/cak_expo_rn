...
export const submitWeeklyReview = createAsyncThunk(
  ...
  async ({rating, skip}: {rating?: number; skip?: boolean}, {rejectWithValue}) => {
    try {
    ...
    builder
      ...
      .addCase(updateTodayWaterIntake.fulfilled, (state, action) => {
        ...
        if (state.homepage && typeof state.homepage === 'object') {
          state.homepage = {
            ...state.homepage,
            water_intake: nextData?.water_intake ?? nextData ?? state.homepage.water_intake,
          };
        }
      })
      ...
      .addCase(updateMealCompletion.fulfilled, (state, action) => {
        ...
        if (state.mealPlan) {
          ...
          if (state.mealPlan.today) updateMealsInList(state.mealPlan.today.meals);
